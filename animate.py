...
def main():
    ...
    SSA_events = Hybrid_data["SSA_events_logs"]
    PDE_update_times = Hybrid_data["PDE_update_times"]
    ...
    time_bins = np.linspace(df['Time'].min(), df['Time'].max(), 256)
    compartment_bins = np.arange(df['Compartment'].min(), df['Compartment'].max() + 1)

    # Create a pivot table for counting events
    heatmap_data = pd.crosstab(
        pd.cut(df['Time'], bins=time_bins), 
        pd.cut(df['Compartment'], bins=compartment_bins)
    )

    # Plotting the heatmap
    # Plotting the heatmap
    plt.figure(figsize=(16, 8))
    ax = sns.heatmap(heatmap_data.T, cmap="YlOrRd", cbar_kws={'label': 'Event Count'}, linewidths=0.1)

    # Improve X-axis (Time)
    time_tick_indices = np.linspace(0, len(time_bins)-2, 11, dtype=int)
    time_tick_labels = [f"{time_bins[i]:.1f}" for i in time_tick_indices]

    ax.set_xticks(time_tick_indices)
    ax.set_xticklabels(time_tick_labels, rotation=45, ha='right')
    ax.set_yticklabels(ax.get_yticklabels(), rotation=0)

    # Overlay PDE update times as vertical lines
    # Overlay PDE update times as vertical lines
    for idx, update_time in enumerate(PDE_update_times):
        ax.axvline(
            x=np.searchsorted(time_bins, update_time) - 0.5,
            color='blue',
            linestyle='--',
            linewidth=1.2,
            alpha=0.7,
            label='PDE Update' if idx == 0 else ""  # Label only the first line
        )

    # Add legend for PDE updates
    handles, labels = ax.get_legend_handles_labels()
    if 'PDE Update' not in labels:
        handles.append(plt.Line2D([], [], color='blue', linestyle='--', label='PDE Update'))
        ax.legend(handles=handles, loc='upper right')

    # Labels and Title
    plt.xlabel('Time')
    plt.ylabel('Compartment')
    plt.title('Event Density Heatmap with PDE Updates')
