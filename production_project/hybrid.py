...
class Hybrid:
    ...
    def __init__(self, domain_length, compartment_number, PDE_multiple, total_time, timestep, threshold, gamma, production_rate, degradation_rate, diffusion_rate, SSA_initial, use_c_functions=False):
        ...
        self.PDE_M = compartment_number * PDE_multiple #Have changed this 02/12/2024 - The coimbined solution is slightly lower, maybe why?
        ...
        self.PDE_initial_conditions = np.ones_like(self.PDE_X, dtype=np.float64)*1000#Initially zero states for PDE
    ...
    def propensity_calculationPython(self, SSA_list: np.ndarray, PDE_list: np.ndarray, production_rate: float, degradation_rate: float) -> np.ndarray:
        """
